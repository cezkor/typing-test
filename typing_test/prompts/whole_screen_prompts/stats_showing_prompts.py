...
class MainStatsScreen(Prompt):
    ...
    def show(self):
        ...
        while True:
            ...
            if c == RawKeyCodes.CTRL_C:
                ...
                if self._do_leave:
                    return
                ss = ShowScoresScreen(self._window, self._colored, self._data)
                ss.traverse()
    ...
    def __print_banner(self):
        ...
        addstr_full_rgls_color(window, self._colored, 2, 3, "Top 10 in highest words per minute:",
                               clp.BLUE_TEXT)
        TabularStatsSubscreen(self._window, self._colored, 4, 3 + 2, self._top_10_by_wpm,
...
class ShowScoresScreen(Prompt):
            ...
            if c == RawKeyCodes.CTRL_C:
                ...
                while do_redraw:
                    ...
                    addstr_full_rgls_color(window, self._colored, y - 2, 3, "Press Ctrl+C to leave".ljust(x-5),
                                           clp.WHITE_TEXT)
                    textField = OnelineTextFieldSubscreen(self._window, self._colored,
    ...
    def __print_main_banner(self):
        ...
        TabularStatsSubscreen(self._window, self._colored, 2, 6, self._pages[self._pages_index],
                              [FDSC.TESTS_BEGAN_ON, FDSC.TOTAL_TIME, FDSC.CATEGORY_OF_TESTS,
                               FDSC.WORD_COUNT_OF_EACH_TEST,
                               FDSC.AVG_WORDS_PER_MINUTE]).print()
    ...
    def __print_scores_banner(self, score_data: list[dict], which: int):
        ...
        if len(self._pages) == 0:
            return
        SingleDataSliceStatsSubscreen(self._window, self._colored, 2, 3, score_data[which]).print()
